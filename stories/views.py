...
def top_stories(top=180, consider=1000):
    ...
    ranked_stories = sorted(
        [(score(story), story) for story in latest_stories],
        reverse=True)
...
def index(request):
    ...
    if request.user.is_authenticated():
        liked_stories = request.user.liked_stories.filter(
            id__in=[story.id for story in stories]
        )
    else:
        liked_stories = []
